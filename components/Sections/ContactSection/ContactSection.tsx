...
export default function ContactSection({
  ...
  useEffect(() => {
    ...
    // NOTE: Empty dependency array - only run once on mount
  }, []);

  // NOTE: Social links with icons for quick contact
  const socialLinks = [
    {
      icon: (
        <svg
      ...
      <div className="container">
        ...
        <div className={`${css.header} ${isVisible ? css.fadeIn : ""}`}>
          <h2 className={css.title}>
            {showViewMore ? "Interested in Collaborating?" : "Let's Connect"}
          </h2>
        ...
        <div className={css.grid}>
          {socialLinks.map((social, index) => (
            ...
            >
              <div className={css.iconWrapper}>{social.icon}</div>
              <span className={css.cardLabel}>{social.label}</span>
            </a>
          ))}
        </div>
